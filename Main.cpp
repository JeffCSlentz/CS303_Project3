...
void decode(string, Binary_Tree<Morse_Data>&); // prints out the decoded message; Pass in a coded word and the appropriate map
void encode(string character, map<string, string>&encode_map);
char findcharacter(string charcode, Binary_Tree<Morse_Data>& tree, int);
	...
	while (index < code.size()){
		if (code[index] == '.' || code[index] == '_' || code[index] == ' '){
			if (code[index] == '.' || code[index] == '_')  // If code isn't a space, save it.
				...
				if (character[0] == '_')
					message += findcharacter(character, decode_tree.get_right_subtree(), 0);
				else message += findcharacter(character, decode_tree.get_left_subtree(), 0);
				character.clear();
			}
...
char findcharacter(string charcode, Binary_Tree<Morse_Data>& tree, int index = 0){
	string currentcode = tree.get_data().code;
	if (currentcode == charcode)
		return tree.get_data().letter;
	else{
		if (charcode[index] == '_')
			return findcharacter(charcode, tree.get_right_subtree(), index++);
		else return findcharacter(charcode, tree.get_left_subtree(), index++);
	}
}